...
//  Partical
//
//  Created by Horton on 1/18/17.
//  Copyright © 2017 YY Inc. All rights reserved.
//

function initPartical(canvasId, particalText, particalImgSrc, imgId) {
	...
	var canvasHandle = {
		...
		_initImageData: function () {
			...
			this.imgy = (canvas.height-img.height)/2 - 100;
			...
			for (var x = 0; x < img.width; x += (particleSize_x+2)) {
		        for (var y = 0; y < img.height; y += (particleSize_y+2)) {
	...
	Particle.prototype = {
		...
		drawSelf: function () {
		    ...
		    ctx.roundRect(this.x - this.width / 2, this.y - this.height / 2, this.width, this.height, this.width);
		    ctx.stroke();
		    ctx.fill();
		},
	...
	function useText(text) {
		...
		if (canvas.width > 520) {
			imgctx.font = "100px Comic Sans MS";
		} else {
			imgctx.font = "60px Comic Sans MS";
		}
		imgctx.fillText(text || 'canvas', img.width/2, img.height/2);
		canvasHandle.init();
	}

	if (particalText) {
		useText(particalText);
	} else if (particalImgSrc && imgId) {
		useImage(src, imgId)
	}
}
